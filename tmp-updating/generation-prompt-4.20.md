# Generation Prompt: Updating Docs 4.20

You are generating updated "Updating clusters" documentation for OpenShift 4.20 based on the 4.19 docs and code diffs.
...
3. Read the 4.19 baseline docs (all assemblies and their included modules).
4. Following Phase 1 of the skill, analyze and map changes to documentation impact.
5. Following Phase 2, generate the complete updated docs for 4.20.
...
- You are NOT writing from scratch. Copy the 4.19 baseline and modify only what the diff demands.
- Version strings: `stable-4.19` → `stable-4.20`, version numbers in examples updated
- Kubelet: check `api.git` go.mod for Kubernetes version → derive kubelet version
- Admin-ack: OCP 4.19 = Kubernetes 1.32 which DOES remove APIs. Check if 4.20's Kubernetes version has removals too (Rule 25)
...
- Check for deprecated content (RHEL compute, KMM) that may need removal (Rule 26)
