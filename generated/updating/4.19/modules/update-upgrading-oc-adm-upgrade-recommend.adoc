...
----
+
Optionally, to enable the alert-based pre-check that evaluates cluster health before recommending updates, also set the following environment variable:
+
[source,terminal]
----
...
----
+
When enabled, the pre-check inspects firing alerts on the cluster and reports conditions that may indicate issues with proceeding to an update, such as critical alerts, PodDisruptionBudget alerts, pod image pull issues, or node readiness problems.
