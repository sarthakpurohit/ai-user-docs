...
[id="sequential-minor-version-updates_{context}"]
== Sequential minor version updates

{product-title} enforces that updates cannot skip minor versions. You must update sequentially through each minor version. For example, you cannot update directly from {product-title} 4.17 to 4.19. You must first update to 4.18, and then update to 4.19. Attempting to apply an update that spans more than one minor version will be blocked by the Cluster Version Operator (CVO).

[id="kube-api-removals_{context}"]
== Kubernetes API removals

There are no Kubernetes API removals in {product-title} 4.19.

// Commenting out this section because there are no APIs being removed in OCP 4.19 / Kube 1.32 that require an admin acknowledgment gate in the CVO.
////
