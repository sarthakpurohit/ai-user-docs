...
[NOTE]
====
With the `ClusterUpdateAcceptRisks` Technology Preview feature gate enabled, you can explicitly accept individual conditional update risks by using the `oc adm upgrade accept` command or by setting the `spec.desiredUpdate.acceptRisks` field in the `ClusterVersion` resource. A conditional update proceeds only when all of its risks have been accepted or are not applicable to the cluster. With the `ClusterUpdatePreflight` Technology Preview feature gate enabled, you can set `spec.desiredUpdate.mode` to `Preflight` to run compatibility checks against a target release without committing to the update. These features require the `TechPreviewNoUpgrade` feature set.
====