...
[NOTE]
====
You can apply a `ConfigMap` named `image-registry-override-drain` in the `openshift-machine-config-operator` namespace to force all image registry changes to skip the drain step. This can be useful when image registry configuration changes do not require a node reboot.
====
...
The `NodeDisruptionPolicy` resource allows you to define how changes to specific files, directories, units, and `sshKeys` are handled on a node. By specifying actions such as `Reload`, `Restart`, `Drain`, `Reboot`, or `None` for particular paths, you can minimize unnecessary node disruptions during configuration updates.

In {product-title} {product-version}, the following default `NodeDisruptionPolicy` entries apply:

* `/etc/containers/registries.d` — Reload `crio`
* `/etc/nmstate/openshift` — `None` (no disruption)
* `/etc/pki/ca-trust/source/anchors/openshift-config-user-ca-bundle.crt` — Restart `coreos-update-ca-trust` and `crio`
