...
|Selects an initial set of optional capabilities to enable. Valid values are `None`, `v4.11`, `v4.12`, `v4.13`, `v4.14`, `v4.15`, `v4.16`, `v4.17`, and `vCurrent`. The default value is `vCurrent`.
|String
...
|String. The name of {gcp-short} project where the image is located.
...
|A list of valid link:https://cloud.google.com/compute/docs/regions-zones#available[{gcp-short} availability zones], such as `us-central1-a`, in a
link:https://yaml.org/spec/1.2/spec.html#sequence//[YAML sequence].
[IMPORTANT]
====
...
|The default disk type for all machines. Control plane nodes must use the `pd-ssd` disk type. Compute nodes can use the `pd-ssd`, `pd-balanced`, `pd-standard`, or `hyperdisk-balanced` disk types.
...
|The {gcp-short} machine type, for example `n1-standard-4`.
...
|The {gcp-short} service account email, for example `<service_account_name>@<project_id>.iam.gserviceaccount.com`.
...
|The {gcp-short} location for the key ring.
...
|`pd-ssd`, `pd-standard`, `pd-balanced`, or `hyperdisk-balanced`. The default is `pd-ssd`.
...
|platform:
  vsphere:
    vcenters:
|Configures the connection details so that services can communicate with a vCenter server. Currently, only a single vCenter server is supported. In Technology Preview, up to 3 vCenter servers are supported.
  ...
  platform:
    nutanix:
      gpus:
        - type:
          name: or deviceID:
|Optional. A list of GPU devices to attach to compute VMs. Each GPU is specified by its device type and identifier. The `type` parameter accepts `name` or `deviceID`. When using `name`, provide the GPU name. When using `deviceID`, provide the integer device ID.
|List of GPU device objects.
    ...
    nutanix:
      dataDisks:
        - diskSize:
          deviceProperties:
            deviceType:
|Optional. A list of additional data disks to attach to compute VMs. Each data disk is defined with a `diskSize` (in Gi) and optional `deviceProperties` for storage configuration.
|List of data disk objects.
