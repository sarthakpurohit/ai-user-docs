...
13. **Do NOT mechanically bump Kubernetes API acknowledgment gates.** The admin-ack (e.g., `ack-4.X-kube-1.Y-api-removals-in-4.Z`) section exists ONLY when Kubernetes actually removes APIs in that release. Check the diff for evidence that a new admin-ack was added:
    ...
    - If there is NO such evidence: **comment out or remove the previous version's ack section entirely.** The previous ack (e.g., `ack-4.15-kube-1.29-api-removals-in-4.16`) is no longer relevant to a cluster already on 4.16+ — it should not appear in 4.17 docs.
    - WRONG: mechanically replacing `ack-4.15-kube-1.29-api-removals-in-4.16` with `ack-4.16-kube-1.30-api-removals-in-4.17`
    - WRONG: leaving the old `ack-4.15-kube-1.29-api-removals-in-4.16` in 4.17 docs unchanged
    - RIGHT: commenting out the ack procedure and adding a note like "There are no Kubernetes API removals in {product-title} 4.17" or removing the section entirely
