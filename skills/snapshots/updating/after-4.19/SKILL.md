...
25. **Kubernetes version → API removal mapping.** The relationship between OpenShift version, Kubernetes version, and API removals is:
    ...
    - Check `api.git` go.mod for `k8s.io/api v0.XX.Y` — the XX is the Kubernetes minor
    - When removals exist, create the full admin-ack section with correct key, removal table, and procedure
